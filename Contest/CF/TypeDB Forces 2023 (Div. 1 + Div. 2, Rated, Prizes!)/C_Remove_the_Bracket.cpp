...
void solve(){
   ...
   fo(2, n -1){
        if(a[i] >= s){
            if(a[i -1] >=s ){
                dp[i][0] = min(dp[i-1][0] + (a[i - 1]- s)*s, dp[i-1][1] + s*s);
                dp[i][1] = min(dp[i-1][0] + (a[i-1] - s) *(a[i] -s), dp[i-1][1] + s*(a[i] -s));
            }else{
                dp[i][0] = min(dp[i-1][0] + a[i-1]*s, dp[i-1][1]);
                dp[i][1] = min(dp[i-1][0] + a[i-1] *(a[i] - s), dp[i-1][1]);
            }
        }else{
            if(a[i - 1] >= s){
                dp[i][0]= min(dp[i-1][0], dp[i-1][1]); 
                dp[i][1] = min(dp[i-1][0] + (a[i-1] - s)*a[i], dp[i-1][1] + s*a[i]);
            }else{
                dp[i][0] = min(dp[i -1][0], dp[i-1][1]);
                dp[i][1] = min(dp[i-1][0] + a[i - 1]*a[i], dp[i-1][1]);
            }
        }
   }
   if(a[n-2] >=s){
    res = min(dp[n-2][0] + (a[n-2]-s)* a[n-1], dp[n-2][1] + s*a[n-1]);
   }
   else{
    res = min(dp[n-2][0] + a[n-2]*a[n-1], dp[n-2][1]);
   }
   print(res);
}
