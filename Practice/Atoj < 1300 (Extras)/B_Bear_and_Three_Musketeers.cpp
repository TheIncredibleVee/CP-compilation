...
void dfs(unordered_set<ll> adj[], ll & res, ll first, ll n){
    ll ans = adj[first].size();
    if(ans < 2){
        return;
    }
    for(auto &second: adj[first]){
        fo(1, n + 1){
            if(i == first || i == second || adj[first].find(i) == adj[first].end() || adj[second].find(i) == adj[second].end()){
                continue;
            }
            res = min(res, ans + (ll)adj[second].size() + (ll)adj[i].size() - 6);
        }
    }
    return;
}
